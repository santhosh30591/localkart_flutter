...
class FlashSaleMoreDetails extends StatefulWidget {
  TodayServiceListModel result;
  int indexs;

  FlashSaleMoreDetails({Key? key, required this.result, required this.indexs})
    : super(key: key);

  @override
  _FlashSaleMoreDetails createState() => _FlashSaleMoreDetails();
}

class _FlashSaleMoreDetails extends State<FlashSaleMoreDetails> {
  ...
  loadingServiceDetails() async {
    ...
    var responces = await HttpClients(
      context,
    ).httpServicesTyepe("viewmegasalespostdetails", inputs);
    try {
      setState(() {
        _isLoading = false;
      });
      ...
      try {
        ...
        try {
          if (datas['errorCode'].toString() == "0") {
            ...
            try {
              stitle = "" + services.result!.shopName!.toString().toUpperCase();
            } catch (e) {
              stitle = "";
            }

            try {
              await services.result!.shopOfferList !=
  ...
  void initState() {
    ...
    print(
      "maga sale postIndexId " +
          servicesMain.result![widget.indexs].postIndexId.toString(),
    );

    // print("Checking array " +
    //     services.result!.shopImageList.runtimeType);
                            ...
                            Container(
                              child: Column(
                                ...
                                children: [
                                  Container(
                                    child:
                                        services.result!.shopOfferList == null
