...
class WeeklyListing extends StatefulWidget {
  dynamic roots;

  WeeklyListing({Key? key, required this.roots}) : super(key: key);

  @override
  _WeeklyListing createState() => _WeeklyListing();
}

class _WeeklyListing extends State<WeeklyListing> {
  @override
  void initState() {
    result.errorCode = 1;
    loadingdetails();
  ...
  getSetvicesList(inputs) async {
    ...
    var responces = await HttpClients(
      context,
    ).httpServicesTyepe("weeklylist", inputs);
    try {
      setState(() {
        _isLoading = false;
      });
