...
import '../../../unit/showingNearMeAletrs.dart';

class FestivalList extends StatefulWidget {
  dynamic roots;

  FestivalList({Key? key, required this.roots}) : super(key: key);

  @override
  _FestivalList createState() => _FestivalList();
}

class _FestivalList extends State<FestivalList> {
  @override
  void initState() {
    result.errorCode = 1;
    loadingdetails();
  ...
  getSetvicesList(inputs) async {
    ...
    var responces = await HttpClients(
      context,
    ).httpServicesTyepe("festivallist", inputs);
    try {
      setState(() {
        _isLoading = false;
      });
